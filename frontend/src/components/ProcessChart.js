...
const ProcessChart = () => {
    ...
    const handleProcessTypeChange = (process) => {setProcessType(process)}

    const handleButtonChange = (config) => {setSimulationConfig(config)}

    useEffect(() => {
        console.log(processType, simulationConfig)
        if(processType){
            getProcess(processType, simulationConfig)
            .then(items => {
      ...
      return(
        <>
            ...
            <ProcessConfig processType={processType} onButtonChange={handleButtonChange}/>
            </div>
            <div className="right-charts-container">
            {simulationResults.map(item => {
                return(
                    <div>
                        <Chart labels={item.times} label={item.name} data={item.results} title={item.title}/>
                    </div>
                )
            })}
            </div>
